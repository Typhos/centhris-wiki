...
import banner from "img/lore/banners/cosmos-banner.jpg";
...
class Cosmos extends Component {
      ...
      <Page.CategoryArticle>
        ...
        <article className="article">
          <section className="article__content">
            {WikiUtils.linkContent(
              ...
              WikiUtils.textFormatting(this.state.articleContent.description),
        ...
        <div className="category" style={{ marginTop: "3em" }}>
          <h2 className="category__subheading">
            ...
            <small>
              ({numberOfArticles}{" "}
              {numberOfArticles > 1 || numberOfArticles === 0 ? "Entries" : "Entry"})
            </small>
          </h2>
          <ul className="category__list">{this.getPlanes(this.state.planes)}</ul>
        </div>
      </Page.CategoryArticle>
    );
  }
  ...
  getPlanes(planes) {
    return planes.sort().map((plane) => {
      return (
        <ListItem
          key={DataLoader.places[plane].name}
          entry={DataLoader.places[plane]}
          imgStyle=""
        />
      );
    });
