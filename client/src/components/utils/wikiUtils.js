...
export default class WikiUtils {
      ...
      entryData = entryData.map((string, i) => {
        ...
        return string.split(/@(.*?)@/).map((substr, j) => {
          ...
          if (/^\*(.*?)\*$/.test(substr)) {
            ...
            substr = (
              <figure
                className={`figureBox ${position ? position : ""}`}
                key={substr + j}
              >
                ...
                <figcaption className='figureBox__caption'>
                  {caption}
                </figcaption>
              </figure>
            );
  ...
  static linkContent(
    ...
    let mapped = descriptionArray.map((paragraph, index) => {
      ...
      paragraph = !Array.isArray(paragraph)
        ? (paragraph = [paragraph])
        : paragraph;
          ...
          if (show || localStorage.getItem("dmView") === "true") {
            ...
            linkingWords.forEach((string, j) => {
              if (string !== undefined && string !== "") {
                const arrayCheck = this.arrayCheck(
                  target,
                ...
                let link = (
                  <Link
                    ...
                    key={`key-${j}-${string}`}
                    to={{
                      pathname: `/${path}/${obj.name.replace(/\s/g, "-")}`,
                      state: "update",
                    }}
                  >
                    {string}
      ...
      if (
        paragraph[0] &&
        (paragraph[0].type === "h3" || paragraph[0].type === "h4")
      ) {
        return paragraph;
      } else {
        return (
          <p
            className={paragraphName ? paragraphName : "linkedContent"}
            key={target + index}
          >
            {paragraph}
          </p>
  ...
  static stubCheck(entry) {
    ...
    let { quote, description, articles } = entry;

    quote = quote || "";
    description = (entry.description && entry.description.join(" ")) || [];
    let articleStr = "";
